...
#########===== CHANGED INT TO FLOAT AND ADDED AN ROUND FUNCTION WHILE PRINTING REMAINING BYTE =====########
import sys
import math
command_txt_sys = "commands1"
command_text = open(command_txt_sys, "r")
num_command_text = sum(1 for line in open(command_txt_sys))
Packet_size = 27.1#float(sys.argv[1]) # ====REVISED===== changed int to float
...
def ROUTE_FINDER(name_of_node):
    ...
    global total_cost
    if nodes_neighbours[name_of_node] in routes:
